...
public class ArticleEsServerImpl implements ArticleEsServer {
    ...
    public void addListener(Message message, Channel channel){
        try {
            ...
            if(Objects.nonNull(message.getBody())){
                ...
                elasticSearchUtils.addDocument(ElasticSearchConstants.BLOG_SEARCH_ARTICLE_INDEX, articleVO.getArticleId().toString(), JsonUtils.objectToJson(articleVO));
    ...
    public void updateListener(Message message, Channel channel){
        try {
            ...
            if(Objects.nonNull(message.getBody())){
                ArticleVO articleVO = JsonUtils.jsonToObject(new String(message.getBody()), ArticleVO.class);
                elasticSearchUtils.updateDocument(ElasticSearchConstants.BLOG_SEARCH_ARTICLE_INDEX, articleVO.getArticleId().toString(), JsonUtils.objectToJson(articleVO));
    ...
    public void deleteListener(Message message, Channel channel){
        try {
            ...
            if(Objects.nonNull(message.getBody())){
                List<Long> articleIds = JsonUtils.jsonToObject(new String(message.getBody()), ArrayList.class);
                if (CollectionUtils.isNotEmpty(articleIds)){
                    for (Long articleId : articleIds){
                        elasticSearchUtils.deleteDocument(ElasticSearchConstants.BLOG_SEARCH_ARTICLE_INDEX, articleId.toString());
                        articleIdsS.append(articleId + ",");
                    }
                }
    ...
    public List<ArticleVO> searchArticleList(String keyword) throws Exception {
        ...
        for(Map<String, Object> x : searchRequests){
            ...
            articleVO.setArticleId(Long.valueOf(x.get("articleId").toString()));
